...
QByteArray SimpleCrypt::encryptToByteArray(QByteArray plaintext)
{
    ...
    if (m_protectionMode == ProtectionChecksum) {
        ...
        s << qChecksum(ba.constData(), uint(ba.length()));
    ...
    resultArray.append(char(CRYPT_VERSION));  //version for future updates to algorithm
...
QByteArray SimpleCrypt::decryptToByteArray(QByteArray cypher)
{
    ...
    if (version != CRYPT_VERSION) {  //we only work with current version
    ...
    if (flags.testFlag(CryptoFlagChecksum)) {
        ...
        ba = ba.mid(2);
        quint16 checksum = qChecksum(ba.constData(), uint(ba.length()));
