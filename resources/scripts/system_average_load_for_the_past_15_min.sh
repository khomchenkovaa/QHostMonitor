#!/bin/sh
#[Name] SYSTEM: Average Load for the past 15 min
...
#[Comment]
#--------------------------------------------------------------------
#Purpose:    Checks an average load of the system during the last 15 min 
#
#Parameters:
...
if [ $# -ge 1 ]
then
  uptime | awk '{load=$(NF)*100; if (load>'$1') {printf("ScriptRes:Bad:%d\n",load)} else {printf("ScriptRes:Ok:%d\n",load)}; exit;}'
else
