...
{
    ui->chkBRecheckDependants->setChecked(s->value(SKEY_HOSTMON_RecheckDependants,SVAL_HOSTMON_RecheckDependants).toInt() == 1);
    ...
    ui->chkBResetOnPause->setChecked(s->value(SKEY_HOSTMON_PauseResetIterations,SVAL_HOSTMON_PauseResetIterations).toInt() == 1);
    ui->chkBRequestForComment->setChecked(s->value(SKEY_HOSTMON_CommentForDisabling,SVAL_HOSTMON_CommentForDisabling).toInt() == 1);
    ui->chkBResetOnDisabled->setChecked(s->value(SKEY_HOSTMON_DisableResetIterations,SVAL_HOSTMON_DisableResetIterations).toInt() == 1);
}
